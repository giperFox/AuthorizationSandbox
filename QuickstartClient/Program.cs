...
namespace QuickstartClient
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //RequestWithClientCredentialsAsync();
            RequestWithUserPasswordAsync();

            Console.ReadKey();
        }

        static async void RequestWithClientCredentialsAsync()
        {
        ...
        static async void RequestWithUserPasswordAsync()
        {
            // discover endpoints from metadata
            var disco = await DiscoveryClient.GetAsync("http://localhost:5000");
